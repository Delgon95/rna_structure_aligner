...
public class GeneticAligner {
	...
	private ArrayList<Specimen> nextGeneration(ArrayList<Specimen> generation) {
		...
		for (int i = 0; i < config.populationSize - size; i++) {
			...
			if (chance <= crossChance) {
				...
				do {
					newSpecimen = new Specimen(config, referenceStructure, targetStructure, isSequenceDependent);
	...
	private ArrayList<Specimen> generatePopulationPool(int populationSize) {
		if (config.geometricPopulation) {
			...
			GeometricAligner aligner = new GeometricAligner(conf_tmp, referenceStructure, targetStructure, isSequenceDependent, rmsdLimit);
			...
			updateStopTime(true);
			updateStopTime(true);
			//System.out.println("Pool:" + res.size() + " elapsed: " + (System.currentTimeMillis() - st));
			return res;
		}
		return null;
	}
	...
	public AlignerOutput calculate() {
		...
		threads.parallel().forEach(index -> {
			...
			while (!terminate && (System.currentTimeMillis() < stopTime)) {
				...
				if (config.geometricPopulation) {
					for (int i = index * config.populationSize; i < (index + 1) * config.populationSize; ++i) {
						Specimen spec;
						...
						if (!population.contains(spec)) {
							spec.calculateRMSD();
							if (index == 0) {
								//System.out.println("SPEC: " + spec.getRmsd() + " " + spec.getUsedNucleotidesNumber());
							}
							population.add(spec);
						}
					}
					...
					while (population.size() < config.populationSize) {
						if (count>10)
							break;
						final Specimen spec = new Specimen(config, referenceStructure, targetStructure, isSequenceDependent);
						spec.initialize(rand.nextInt(85) + 5);
						spec.refinement();
				...
				double best_incorrectlyAlignedResiduesRatio = population.get(0).getIncorrectlyAlignedResiduesRatio();
				Specimen best_specimen = (Specimen) population.get(0).clone();
				//System.out.println("Thread: " + index + " " + best_size + " " + best_rmsd);
				if (config.geometricPopulation) {
					try {
						semaphore.acquire();
						if ((best_rmsd <= rmsdLimit && best_size > bestAlignmentSize) ||
							((Double.compare(best_size, bestAlignmentSize) == 0) && (best_rmsd < bestAlignmentRMSD))) {
							//updateStopTime(bestAlignmentSize < best_size);
							//System.out.println("Improved result 2 " + bestAlignmentSize + " -> " + best_size + " | " + bestAlignmentRMSD + " -> " + best_rmsd);
				...
				if (!((best_size == Math.min(referenceStructure.size(), targetStructure.size()))
					...
					while ((System.currentTimeMillis() - last_improvement < 1000 * config.resetThreadTime 
						...
						final double incorrectlyAlignedResiduesRatio = population.get(0).getIncorrectlyAlignedResiduesRatio();
						//System.out.println(currentSize + ", " + currentRmsd + ", " + incorrectlyAlignedResiduesRatio + ", " + numberOfPopulationsWithoutImprovement);
						if (((best_incorrectlyAlignedResiduesRatio > incorrectlyAlignedResiduesRatio) && (Double.compare(currentRmsd,rmsdLimit) <= 0)) || 
								((Double.compare(best_incorrectlyAlignedResiduesRatio,incorrectlyAlignedResiduesRatio) == 0) && (best_size < currentSize) && (Double.compare(currentRmsd,rmsdLimit) <= 0)) || 
								((Double.compare(best_incorrectlyAlignedResiduesRatio,incorrectlyAlignedResiduesRatio) == 0) && (best_size == currentSize) && (best_rmsd > currentRmsd))) {
							...
							if (bestAlignmentSize < best_size || (best_rmsd < bestAlignmentRMSD && bestAlignmentSize == best_size)) {
								...
								try {
									...
									if ((best_rmsd <= rmsdLimit && best_size > bestAlignmentSize) ||
											((Double.compare(best_size, bestAlignmentSize) == 0) && (best_rmsd < bestAlignmentRMSD))) {
										updateStopTime(bestAlignmentSize < best_size);
										//System.out.println("Improved result 1 " + bestAlignmentSize + " -> " + best_size + " | " + bestAlignmentRMSD + " -> " + best_rmsd);
										bestAlignmentSize = best_size;
										bestAlignmentRMSD = best_rmsd;
										bestAlignmentIncorrectlyAlignedResiduesRatio = best_incorrectlyAlignedResiduesRatio;
										bestSpecimen = (Specimen) best_specimen.clone();
				...
				try {
					...
					if (((bestAlignmentSize == Math.min(referenceStructure.size(), targetStructure.size()))
							&& (Double.compare(bestAlignmentRMSD,rmsdLimit) <= 0)) || (numberOfPopulationsWithoutImprovement > 300))
						terminate = true;
				} catch (InterruptedException e) {
					LOGGER.error(e.getMessage(), e);
				} finally {
		...
		//System.out.println(bestSpecimen.primaryNucleotidesUsed.length);
		...
		for (int i = 0; i < bestSpecimen.primaryNucleotidesUsed.length; ++i) {
			...
			//System.out.println(i + ": " + bestSpecimen.primaryNucleotidesUsed[i] + " (" + bestSpecimen.secondaryNucleotidesMap[i] + ")");
		}
		...
		return new AlignerOutput(bestSpecimen.getUsedNucleotidesNumber(), referenceIndexes, targetMapping,
				Calculations.FitForRMSD(
						...
						Nucleotide.NucleotidesToListMapped(bestSpecimen.secondaryNucleotides,
								bestSpecimen.secondaryNucleotidesMap, false)),
