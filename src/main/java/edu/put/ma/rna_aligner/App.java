...
public class App {
	...
	public static void main(final String[] args) {
		...
		if (args[0].equals("genetic")) {
			GeneticAligner aligner = new GeneticAligner(config, referenceStructure, targetStructure, isSequenceDependent, rmsdLimit);
			output = aligner.calculate();
		} else {
			GeometricAligner aligner = new GeometricAligner(config, referenceStructure, targetStructure, isSequenceDependent, rmsdLimit);
			output = aligner.calculate();
		}

		final String modelNameWithoutExtension = FilenameUtils.removeExtension(FilenameUtils.getName(args[2]));

		final StringBuilder outputStringBuilder = new StringBuilder();
		outputStringBuilder
				.append(String.format("Aligning mode: %s\n", ((isSequenceDependent) ? "sequence-dependent" : "sequence-independent")));
		outputStringBuilder.append(String.format("Maximal RMSD threshold: %.2f\n", rmsdLimit));
		outputStringBuilder
				.append(String.format("Residues number of reference structure: %d\n", referenceStructure.size()));
		outputStringBuilder.append(String.format("Residues number of model: %d\n", targetStructure.size()));

		if ((output != null) && (output.aligned > 0)) {
			outputStringBuilder.append(String.format("Number of aligned nucleotides: %d\n", output.aligned));
			outputStringBuilder.append(String.format("RMSD score: %.3f\n", output.rmsd));
			outputStringBuilder.append(String.format("Processing time [ms]: %d\n", output.processingTime));
		} else
			outputStringBuilder.append(String.format("Alignment is not found.\n"));

		saveDataToFile(
				new StringBuilder(outputDirectory.getAbsolutePath()).append(File.separator)
						.append(modelNameWithoutExtension).append("-output.txt").toString(),
				outputStringBuilder.toString());

		if ((output != null) && (output.aligned > 0)) {

			final StringBuilder alignedResiduesMappingStringBuilder = new StringBuilder("REF  \t<->\tMODEL\n");
			for (int i = 0; i < output.referenceIndexes.size(); ++i) {
				final int targetIndex = output.targetMapping.get(i).intValue();
				alignedResiduesMappingStringBuilder.append(
						String.format("%s\t<->\t%s\n", referenceStructure.get(output.referenceIndexes.get(i)).getKey(),
								(targetIndex > -1) ? targetStructure.get(targetIndex).getKey() : "-"));
			}
			saveDataToFile(
					new StringBuilder(outputDirectory.getAbsolutePath()).append(File.separator)
							.append(modelNameWithoutExtension).append("-residue-mapping.txt").toString(),
					alignedResiduesMappingStringBuilder.toString());

			saveDataToFile(
					new StringBuilder(outputDirectory.getAbsolutePath()).append(File.separator)
							.append(modelNameWithoutExtension).append("-sequence-alignment.txt").toString(),
			...
			saveDataToFile(new StringBuilder(outputDirectory.getAbsolutePath()).append(File.separator)
					.append(modelNameWithoutExtension).append("-superimposed.pdb").toString(), model.toPDB());

			final StringBuilder refModelSuperimpositionStringBuilder = new StringBuilder();
			refModelSuperimpositionStringBuilder
					.append("MODEL        1                                              \n");
			...
			refModelSuperimpositionStringBuilder
					.append("ENDMDL                                                      \n");
			refModelSuperimpositionStringBuilder.append("END                                                         ");

			final String referenceNameWithoutExtension = FilenameUtils.removeExtension(FilenameUtils.getName(args[1]));
			saveDataToFile(
					new StringBuilder(outputDirectory.getAbsolutePath()).append(File.separator)
							.append(referenceNameWithoutExtension).append("-").append(modelNameWithoutExtension)
							.append("-superimposed.pdb").toString(),
	...
	private static final void validateInput(final String[] args,final AlignerConfig _config) {
		if (args.length < 5 || args.length > 6) {
			LOGGER.info("Usage: <method> <reference.pdb> <target.pdb> <aligning-mode> <rmsd-threshold> <output-directory> (optional)");
			LOGGER.info("Provided methods: geometric, genetic");
			LOGGER.info("Provided aligning modes: sequence-dependent (seq-dep), sequence-independent (seq-indep). By default sequence-dependent is used.");
			LOGGER.info("RMSD threshold: maximal RMSD score of resultant alignment. By default 3.5A is used.");
			System.exit(1);
		}

		if ((StringUtils.equalsIgnoreCase(args[0], "geometric")) || (StringUtils.equalsIgnoreCase(args[0], "genetic"))) {
	...
	private static final void rotateAndShift(final Structure structure, final Superimposer superimposer) {
